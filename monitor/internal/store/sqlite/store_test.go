...
func TestAddFriction_Basic(t *testing.T) {
	...
	// Query back — should aggregate to magnitude 10, count 1, for each law.
...
func TestAddFriction_NoLaws(t *testing.T) {
	...
	if results[0].TotalMagnitude != 5 {
		t.Fatalf("expected magnitude 5, got %f", results[0].TotalMagnitude)
	}
}
...
func TestQueryFriction_Aggregation(t *testing.T) {
	...
	events := []struct {
		...
		{"evt-1", 10, []string{"law-1"}},
		{"evt-2", 20, []string{"law-1"}},
		{"evt-3", 5, []string{"law-2"}},
	}
	...
	if results[0].TotalMagnitude != 30 {
		t.Fatalf("expected total magnitude 30 for law-1, got %f", results[0].TotalMagnitude)
	}
...
func TestQueryFriction_MultipleLawsPerEvent(t *testing.T) {
	...
	for _, r := range results {
		if r.TotalMagnitude != 15 {
			t.Errorf("expected magnitude 15 for law %q, got %f", r.LawID, r.TotalMagnitude)
		}
		if r.EventCount != 1 {
...
func TestQueryFriction_FilterByNode(t *testing.T) {
	...
	if results[0].TotalMagnitude != 10 {
		t.Fatalf("expected magnitude 10 for node-a, got %f", results[0].TotalMagnitude)
	}
}
...
func TestQueryFriction_FilterByTimeRange(t *testing.T) {
	...
	if results[0].TotalMagnitude != 30 {
		t.Fatalf("expected magnitude 30 (10+20), got %f", results[0].TotalMagnitude)
	}
	if results[0].EventCount != 2 {
...
func TestRecordTelemetry_Basic(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	...
	err := s.RecordTelemetry(ctx, event)
	if err != nil {
...
func TestRecordTelemetry_PayloadTooLarge(t *testing.T) {
	...
	largePayload := make([]byte, 65537) // 64KB + 1
