...
**Workitem transfer** is handled by the Embassy. Every Flow has an operator-provisioned Embassy instance that manages outbound export and inbound import. Embassy resolves imports against one effective import-type namespace composed of platform-owned system import types plus flow-authored `crossFlow.importTypes`. Senders target stable public import type names, never internal node names. `law-petition` is the only currently defined built-in system import type and is always present/configured per Flow rather than authored in YAML. Federation-member and Treaty-based exchange use the same Embassy manifest + package protocol; only the trust source differs.
