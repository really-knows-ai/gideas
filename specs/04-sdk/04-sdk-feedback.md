...
The Arbiter fans out to [Juror nodes](../01-concepts/02-foundry-cycle.md#juror-judicial-agent) for multi-agent deliberation. The [Deliberation Gate](../01-concepts/02-foundry-cycle.md#deliberation-gate-consensus-tally) tallies juror verdicts and determines consensus. On consensus (or after HITL resolution of a hung jury), the verdict flows to the [Clerk node](../01-concepts/02-foundry-cycle.md#clerk-petition-drafter) to draft a Tier 2 Ruling as a [petition](../01-concepts/02-foundry-cycle.md#petition-artefact). The Arbiter renders the verdict:

- **Verdict favours refiner** — the feedback state is set to `wont_fix` with `linkedRuling` pointing to the Tier 2 Ruling (drafted by the [Clerk node](../01-concepts/02-foundry-cycle.md#clerk-petition-drafter) as a petition) that captures the decision. The reviewing node must call `AcceptRefusal()`.
...
In both cases, [Juror deliberation friction](../01-concepts/03-data-model.md#friction) is emitted: each deliberation round produces friction at magnitude `depth ^ (round + 1)`. If the [Deliberation Gate](../01-concepts/02-foundry-cycle.md#deliberation-gate-consensus-tally) returns a hung verdict and the Arbiter escalates to the [Advocate](../02-flow/03-nodes-external.md#the-judiciary--standard-subsystem) for human intervention, a single friction event is emitted at magnitude `depth ^ (rounds * 2)`.
