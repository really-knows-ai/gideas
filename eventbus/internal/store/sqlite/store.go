...
type Store struct {
	db  *sql.DB
	mu  sync.Mutex
	seq map[int32]uint64 // per-channel sequence counters
}
...
func New(dsn string) (*Store, error) {
	...
	s := &Store{db: db, seq: make(map[int32]uint64)}
...
func (s *Store) Insert(ctx context.Context, evt *Event) (uint64, error) {
	...
	if err != nil {
		// Roll back the sequence on failure.
		s.seq[evt.Channel]--
...
func (s *Store) GetSince(ctx context.Context, channel int32, lastSequence uint64, limit int) ([]Event, error) {
...
func (s *Store) MinSequence(ctx context.Context, channel int32) (uint64, error) {
...
// until the channel's total payload is under the size limit.
func (s *Store) Evict(ctx context.Context, channel int32, maxAge time.Duration, maxBytes int64) (int64, error) {
