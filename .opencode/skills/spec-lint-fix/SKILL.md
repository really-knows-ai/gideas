...
It runs the repository linter from `tools/spec-lint/`, fixes reported issues in markdown files, and repeats until `pnpm lint` exits clean.
...
2. Run `pnpm lint` in `tools/spec-lint/`.
...
4. Re-run `pnpm lint` in `tools/spec-lint/`.
