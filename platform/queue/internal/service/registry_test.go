...
func TestRegisterQueue_CreatesCR_OnFirstRegistration(t *testing.T) {
	...
	resp, err := r.RegisterQueue(ctx, &flowv1.RegisterQueueRequest{
		QueueName: "hitl-approval", ShardId: "shard-0", ShardAddr: testShard0Addr,
	})
	if err != nil {
	...
	if q.Status.Shards[0].ShardID != "shard-0" || q.Status.Shards[0].Addr != testShard0Addr {
...
func TestRegisterQueue_UpsertsShard_OnExistingCR(t *testing.T) {
	...
	seed := queueCR("hitl-approval", shard("shard-0", testShard0Addr, phaseActive, now))
...
func TestHeartbeatQueue_UpdatesLastHeartbeatAt(t *testing.T) {
	...
	seed := queueCR("hitl-approval", shard("shard-0", testShard0Addr, phaseActive, old))
	c := newFakeClient(t, seed)
	r := NewRegistry(c, time.Second, time.Second)
	r.Namespace = testNamespace
...
func TestHeartbeatQueue_ReturnsLivingShardSet(t *testing.T) {
	...
	seed := queueCR(
		"hitl-approval",
		shard("shard-0", testShard0Addr, phaseActive, now),
	...
	if got["shard-0"] != testShard0Addr || got["shard-1"] != "10.0.0.2:50053" {
...
func TestDeregisterQueue_RemovesShard_LeavesOthers(t *testing.T) {
	...
	if _, err := r.DeregisterQueue(ctx, &flowv1.DeregisterQueueRequest{
		QueueName: "hitl-approval", ShardId: "shard-0",
	}); err != nil {
...
func TestListQueues_ReturnsRegisteredQueues(t *testing.T) {
	...
	now := time.Now().UTC()
	c := newFakeClient(t,
		queueCR("hitl-approval", shard("shard-0", testShard0Addr, phaseActive, now)),
