...
//
// Responsibilities:
//   - Deploy and manage pods identical to FlowSupportService lifecycle.
//   - The provided capability is always "encode" (implicit).
//   - Track phase and available replicas.
...
// Reconcile creates or updates the workload for the CodificationService
...
func (r *CodificationServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the CodificationService instance.
	var svc flowv1.CodificationService
	if err := r.Get(ctx, req.NamespacedName, &svc); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	log.Info("Reconciling CodificationService",
		"name", svc.Name,
		"namespace", svc.Namespace,
		"outputFormat", svc.Spec.OutputFormat,
	)
	...
	reason := "Reconciled"
	message := fmt.Sprintf("CodificationService reconciled with %d available replicas", availableReplicas)
	if phase != phaseReady {
...
func (r *CodificationServiceReconciler) determinePhase(svc *flowv1.CodificationService, available int32) string {
	minReplicas := int32(0)
...
// reconcileDeployment creates or updates a Deployment for the CodificationService.
func (r *CodificationServiceReconciler) reconcileDeployment(ctx context.Context, svc *flowv1.CodificationService) (int32, error) {
	replicas := int32(1)
...
// reconcileStatefulSet creates or updates a StatefulSet for the CodificationService.
func (r *CodificationServiceReconciler) reconcileStatefulSet(ctx context.Context, svc *flowv1.CodificationService) (int32, error) {
	replicas := int32(1)
	if svc.Spec.MinReplicas != nil && *svc.Spec.MinReplicas > 0 {
		replicas = *svc.Spec.MinReplicas
	}
...
// buildPodTemplate constructs the pod template spec for the codification service.
func (r *CodificationServiceReconciler) buildPodTemplate(svc *flowv1.CodificationService, labels map[string]string) corev1.PodTemplateSpec {
	container := corev1.Container{
		Name:  "codification-service",
		Image: svc.Spec.Image,
		Env: []corev1.EnvVar{
			{Name: "OUTPUT_FORMAT", Value: svc.Spec.OutputFormat},
		},
	}
...
// persistStatus re-fetches and persists the CodificationService status.
func (r *CodificationServiceReconciler) persistStatus(
	ctx context.Context,
	...
	var fresh flowv1.CodificationService
...
// labelsForService returns standard labels for resources owned by this CodificationService.
func (r *CodificationServiceReconciler) labelsForService(svc *flowv1.CodificationService) map[string]string {
	return map[string]string{
		"app.kubernetes.io/name":       "codificationservice",
		"app.kubernetes.io/instance":   svc.Name,
		"app.kubernetes.io/managed-by": "foundry-operator",
		"flow.gideas.io/codification":  svc.Name,
	}
}
